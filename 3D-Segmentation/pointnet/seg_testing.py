...
def _arg_parsing():
    parser = argparse.ArgumentParser("Testing Semantic Segmentation")
...
class PredictSemSeg():
    def __init__(self):
        self.root = '/home/reza/Desktop/thesis_tum/dataset/dummy_stanford_indoor3d/'
        self.parser_obj = _arg_parsing()


    def folder_structures(self):
        os.environ["CUDA_VISIBLE_DEVICES"] = self.parser_obj.gpu
        experiment_dir = 'log/sem_seg/' + self.parser_obj.log_dir
...
def parse_args():
    ...
    parser.add_argument('--num_point', type=int, default=128, help='Point Number [default: 4096]')
    parser.add_argument('--log_dir', type=str, default='s3dis_log', help='Experiment root')
    parser.add_argument('--visual', action='store_true', default=True, help='Whether visualize result [default: False]')
    parser.add_argument('--test_area', type=int, default=5, help='Which area to use for test, option: 1-6 [default: 5]')
    parser.add_argument('--num_votes', type=int, default=5, help='Aggregate segmentation scores with voting [default: 5]')
    return parser.parse_args()
...
def main(args):
    ...
    experiment_dir = '/home/reza/Desktop/thesis_tum/nn_training_stuffs/log_directory/semantic_segmentation/' + args.log_dir
    visual_dir = experiment_dir + '/visual/'
    visual_dir = Path(visual_dir)
    visual_dir.mkdir(exist_ok=True)
    ...
    root = '/home/reza/Desktop/thesis_tum/dataset/dummy_stanford_indoor3d/'

    TEST_DATASET_WHOLE_SCENE = ScannetDatasetWholeScene(root, split='test', test_area=args.test_area, block_points=NUM_POINT)
    log_string("The number of test data is: %d" %  len(TEST_DATASET_WHOLE_SCENE))

    '''MODEL LOADING'''
    model_name = importlib.import_module('.semantic_seg_model', package='pointnet')
    #model_name = os.listdir(experiment_dir+'/logs')[0].split('.')[0]
    #MODEL = importlib.import_module(model_name)
    classifier = model_name.ModelCreation(NUM_CLASSES).cuda()
